	...
	// 
 	for key, tag := range inputTags {
		if tag != "" {
			...
			windowStartTime := time.Date(2019, 9, 26, 23, 33, 58, 060000000, time.UTC)
			...
			for _, wr := range windowResult {
				...
				if err != nil {
					activityLog.Infof("[kxanalogavg] time  is invalid %s for tag %s - skipped", wr["time"], tag)
					continue
				}
				t := time.Unix(0, tint)
			...
			for i, v := range avgData {
				...
				activityLog.Infof("Time %d, Diff: %f, Value %f, Avg: %f", v.tim.UnixNano(), diff, v.val, diff * prevVal / float64(totalInterval))
